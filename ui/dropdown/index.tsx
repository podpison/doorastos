...
const Dropdown: FC<DropdownProps> = ({
  ...
  const handleItemClick = (item: string) => {
    ...
    if (activeItems.includes(formattedItem)) {
      setActiveItem(
        ...
        activeItems.filter((fi) => fi !== formattedItem)
      );
    } else {
      setActiveItem(defaultItem.toLowerCase(), [...activeItems, formattedItem]);
    }
  };
  ...
  return (
    <div
      className={cn(
        "relative border border-grey4 w-full",
        isExpanded ? "border-b-0" : "transition-colors hover:border-grey2"
      )}
      ref={dropdownRef}
    >
      <button
