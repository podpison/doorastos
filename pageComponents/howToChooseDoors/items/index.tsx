...
import ButtonsGroup, {
  ButtonsGroupItemType,
} from "@/ui/buttonsGroup/buttonsGroup";

export type HowToChooseDoorsItemsType = (
  | HowToChooseDoorsTextItemProps
  | HowToChooseDoorsImgItemProps
)[];
...
const isTextItemType = (
  item: HowToChooseDoorsTextItemProps | HowToChooseDoorsImgItemProps
): item is HowToChooseDoorsTextItemProps => {
...
const Items: FC<Props> = ({ categories = [], where = 'apartment' }) => {
  let currentCategory =
    categories.find((c) => c.category === where) || categories[0];
  let Items = currentCategory.items.map((d, index) =>
    isTextItemType(d) ? (
      <TextItem {...d} key={index} />
    ) : (
      <ImgItem {...d} key={index} />
    )
  );
  ...
  return (
    <div>
      <ButtonsGroup
        className="grid grid-cols-[max(160px,_50%)_1fr] esm:grid-cols-2 md:max-w-[400px]"
