      ...
      texts: [
        {
          ...
          description:
            "This is a layer of material with low thermal conductivity, separating the outer and inner sides of the structure so that one does not transfer its temperature to the other.",
        },
        {
  ...
  },
  {
    ...
    info: {
      ...
      texts: [
        {
          heading:
            "Burglary-resistant entrance doors are steel doors endowed with strength characteristics and special technological solutions to protect against forced and intelligent burglary and opening.",
          description:
            'These doors are "Burglary Resistant" only after passing the proper tests in a special laboratory, which determines how the front door is able to resist burglary by special metal-cutting and destructive tools, as well as pyrotechnics for a certain time.',
        },
      ],
    },
  },
  {
    ...
    info: {
      ...
      texts: [
        {
          heading:
            "Armored doors are single or double doors with a metal frame covered with MDF. They vary in complexity of design and price.",
          description: [
...
const SecurityProperties: FC<Props> = ({ activeItem }) => {
  const router = useRouter();
  
  let Items = securityProperties.map((i, index) => (
    <Item
  ...
  return (
    <div className="grid gap-y-5 smlg:grid-cols-[max-content_1fr_1fr] smlg:gap-x-2.5 lg:grid-cols-[40%_1fr_1fr] lg:gap-x-4 xl:grid-cols-3">
      {Items}
    </div>
  );
};
