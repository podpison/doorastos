...
const Item: FC<Props> = ({ name, imgs, price, material, id, discount, isUnique, onClick }) => {
  let finalPrice = getPriceWithDiscount(price, discount?.value);

  return <Link className='group' href={`/catalog/${textToURL(name)}`} data-price={finalPrice} aria-label={`${name} door`} onClick={onClick}>
